...
import type { PodcastCommentData } from "@/features/podcast-cms/interfaces";
...
function CommentItem({
  ...
  const deleteComment = useDeleteEpisodeComment();

  const handleReply = () => {
    if (!replyContent.trim()) return;
    createComment.mutate(
      { episodeId, content: replyContent.trim(), parentId: comment.id },
      {
        onSuccess: () => {
          setReplyContent("");
          setReplyOpen(false);
  ...
  const handleDelete = () => {
    deleteComment.mutate({ commentId: comment.id, episodeId });
  };
  ...
  return (
    <div
      className={depth > 0 ? "ml-10 border-l-2 border-default-200 pl-4" : ""}
    >
          ...
          <div className="flex items-center gap-1 mt-2">
            {currentUserId && depth < 2 && (
              <Button
                ...
                size="sm"
                startContent={
                  <Icon icon="solar:reply-linear" className="text-base" />
                }
...
export default function EpisodeComments({ episodeId }: EpisodeCommentsProps) {
  const { user, loading: userLoading, localUser } = useGetIdentity();
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    createComment.mutate(
      { episodeId, content: newComment.trim() },
      {
    ...
    <div className="py-8">
      ...
      {userLoading ? (
        ...
        <div className="flex gap-3 mb-8">
          <Avatar
            ...
            name={user?.name || localUser?.name || "U"}
            size="sm"
        ...
        <div className="mb-8 p-4 rounded-xl bg-default-50 border border-default-200 text-center">
          <div className="flex flex-col items-center gap-2">
            ...
            <Button
              as="a"
              href={`/sign-in?redirect_url=${typeof window !== "undefined" ? window.location.pathname : ""}`}
              size="sm"
              variant="flat"
              color="primary"
            >
      ...
      {isLoading ? (
        ...
        <StateMessage
          ...
          message="Be the first to share your thoughts on this episode."
