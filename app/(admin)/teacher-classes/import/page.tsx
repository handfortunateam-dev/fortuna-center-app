...
export default function ImportTeacherClassesPage() {
  const resource = "teacher-classes";
  const resourceLabel = "Teacher Classes";
  ...
  // Mode: "file" = from Excel, "manual" = add rows manually
    ...
    const fetchData = async () => {
      ...
      try {
        const [teacherRes, classRes] = await Promise.all([
          fetch("/api/users?source=db&limit=1000&role=TEACHER"),
          fetch("/api/classes"),
        ]);
        const teacherData = await teacherRes.json();
        const classData = await classRes.json();

        if (teacherData.success && Array.isArray(teacherData.data)) {
          setTeachers(
            teacherData.data.map((t: Record<string, string>) => ({
              id: t.id,
        ...
        Toast({
          title: "Error",
          description: "Failed to load teachers and classes data.",
          color: "warning",
        });
      } finally {
  ...
  const handleAddRow = () => {
    setAssignments((prev) => [
      ...prev,
      { key: `manual-${Date.now()}`, teacherId: "", classId: "" },
    ]);
  };

  const handleRemoveRow = (key: string) => {
    setAssignments((prev) => prev.filter((r) => r.key !== key));
  };

  const handleAssignmentChange = (
    key: string,
    field: "teacherId" | "classId",
    value: string,
  ) => {
    setAssignments((prev) =>
  ...
  const handleUpload = async () => {
    ...
    if (mode === "file") {
      ...
      const validRows = assignments.filter((r) => r.teacherId && r.classId);
      if (validRows.length === 0) {
        Toast({
          title: "Error",
          description: "No valid assignments. Please select both Teacher and Class.",
          color: "danger",
        });
        return;
      }
    ...
    <div className="p-6 max-w-[95%] mx-auto space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          Assign guru ke kelas. Pilih dari dropdown atau upload file template Excel.
        </p>
      </div>
      ...
      {importResult ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card
              ...
              onPress={() => {
                setMode("manual");
                setAssignments([{ key: `manual-${Date.now()}`, teacherId: "", classId: "" }]);
              }}
              ...
              <CardBody className="flex flex-col items-center gap-4 p-8">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Pilih teacher dan class langsung dari dropdown
                  </p>
                </div>
              </CardBody>
            </Card>
            ...
            <CardBody className="flex flex-row items-start gap-4 p-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-blue-600/80 dark:text-blue-400/80">
                  Template sudah berisi dropdown Teacher dan Class dari database.
                </p>
                <Button
                  size="sm"
                  variant="light"
        ...
        <div className="space-y-6">
          <Card className="border border-gray-200 dark:border-gray-800 shadow-sm">
            <CardHeader className="flex gap-3 px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
              ...
              <div className="flex flex-col flex-1">
                <p className="text-md font-semibold">Assign Teachers to Classes</p>
                <p className="text-small text-default-500">
                  Pilih teacher dan class dari dropdown
                </p>
              </div>
              <Button
                size="sm"
            ...
            <CardBody className="px-6 py-4 gap-4">
              {isLoadingData ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <span className="ml-3 text-sm text-gray-500">Loading teachers & classes...</span>
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <select
                          value={row.teacherId}
                          onChange={(e) =>
                            handleAssignmentChange(row.key, "teacherId", e.target.value)
                          }
                      ...
                      <div className="flex-1">
                        ...
                        <select
                          value={row.classId}
                          onChange={(e) =>
                            handleAssignmentChange(row.key, "classId", e.target.value)
                          }
                          className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary"
                        >
          ...
          <div className="flex justify-end items-center gap-4 p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm sticky bottom-4 z-20">
            ...
            >
              Confirm Import ({assignments.filter((r) => r.teacherId && r.classId).length} assignments)
            </Button>
          </div>
        </div>
      ) : (
